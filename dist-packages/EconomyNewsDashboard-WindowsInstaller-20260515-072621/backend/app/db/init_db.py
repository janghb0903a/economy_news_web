...
def create_fts_tables() -> None:
    ...
    with engine.begin() as conn:
        ...
        if "duplicate_of_id" not in columns:
            conn.execute(text("ALTER TABLE articles ADD COLUMN duplicate_of_id INTEGER"))
        if "duplicate_marked_at" not in columns:
            conn.execute(text("ALTER TABLE articles ADD COLUMN duplicate_marked_at DATETIME"))
