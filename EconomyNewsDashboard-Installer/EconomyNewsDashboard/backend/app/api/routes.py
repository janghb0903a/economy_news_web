...
from app.models.entities import AppSetting, Article, FetchLog, Report, Source, UserKeyword
...
LATEST_ARTICLES_SEEN_SETTING_KEY = "latest_articles_seen_map"
LATEST_ARTICLES_SEEN_MAX = 500
...
class UserKeywordUpdatePayload(BaseModel):
    active: bool | None = None


class UserKeywordRead(BaseModel):
...
class KeywordRecommendationRead(BaseModel):
    keyword: str
    count: int


class KeywordDictionaryRead(BaseModel):
    items: list[UserKeywordRead]
    recommendations: list[KeywordRecommendationRead]


class LatestArticleSeenRead(BaseModel):
    seen: dict[str, int]


class LatestArticleSeenUpdate(BaseModel):
...
KEYWORD_RECOMMENDATION_STOPWORDS = {
    "기사",
    "뉴스",
    "경제",
    "국내",
...
def normalize_user_keyword(keyword: str) -> str:
    return re.sub(r"\s+", " ", (keyword or "").strip().lstrip("#")).strip()


def valid_user_keyword(keyword: str) -> bool:
    normalized = normalize_user_keyword(keyword)
    if len(normalized) < 2 or len(normalized) > 40:
        return False
    lower = normalized.lower()
    if lower.isdigit() or lower in KEYWORD_RECOMMENDATION_STOPWORDS or lower in TITLE_STOPWORDS:
        return False
    return bool(re.search(r"[A-Za-z0-9가-힣]", normalized))


def article_keyword_search_text(article: Article) -> str:
    parts = [
        article.title,
        ...
        article.content,
        article.tags_text,
        article.bok_keywords_json,
        article.bok_keyword_groups_json,
        article.source_name,
        article.category,
    ]
...
def article_keyword_candidates(article: Article) -> list[str]:
    candidates: list[str] = []
    candidates.extend(item.strip().lstrip("#") for item in (article.tags_text or "").split(",") if item.strip())
    candidates.extend(str(item).strip() for item in parse_json_list(article.bok_keywords_json) if str(item).strip())
    candidates.extend(str(item).strip() for item in parse_json_list(article.bok_keyword_groups_json) if str(item).strip())
    candidates.extend(ordered_title_tokens(article.title))
    candidates.extend(ordered_title_tokens(article.translated_title or ""))

    result: list[str] = []
    seen: set[str] = set()
    for candidate in candidates:
        normalized = normalize_user_keyword(candidate)
        lower = normalized.lower()
        if not valid_user_keyword(normalized) or lower in seen:
            continue
        seen.add(lower)
...
def active_keyword_counts(db: Session, entries: list[UserKeyword] | None = None, hours: int = 24) -> dict[int, int]:
    entries = entries if entries is not None else db.query(UserKeyword).filter(UserKeyword.active.is_(True)).all()
    if not entries:
        return {}
    cutoff = kst_now() - timedelta(hours=hours)
    recent_articles = (
        db.query(Article)
        .filter(func.coalesce(Article.published_at, Article.fetched_at) >= cutoff)
    ...
    for article in recent_articles:
        text_value = article_keyword_search_text(article)
...
def keyword_recommendations(db: Session, limit: int = 20) -> list[dict[str, int | str]]:
    existing = {keyword.lower() for (keyword,) in db.query(UserKeyword.keyword).all()}
    counter: Counter[str] = Counter()
    saved_articles = (
        db.query(Article)
        .filter(Article.is_saved.is_(True))
        .order_by(func.coalesce(Article.published_at, Article.fetched_at).desc(), Article.id.desc())
        .limit(500)
        .all()
    )
    for article in saved_articles:
        for keyword in article_keyword_candidates(article):
            if keyword.lower() not in existing:
                counter[keyword] += 1
    return [{"keyword": keyword, "count": count} for keyword, count in counter.most_common(limit)]


def automatic_recent_keywords(db: Session, hours: int = 24, limit: int = 20) -> list[dict[str, int | str]]:
    cutoff = kst_now() - timedelta(hours=hours)
    articles = (
        db.query(Article)
        .filter(func.coalesce(Article.published_at, Article.fetched_at) >= cutoff)
    ...
    for article in articles:
        ...
        for keyword in article_keyword_candidates(article):
            ...
            counter[key] += 1
    return [{"name": display_names[key], "count": count} for key, count in counter.most_common(limit)]


def user_keyword_read(entry: UserKeyword, recent_count: int = 0) -> UserKeywordRead:
    return UserKeywordRead(
        id=entry.id,
...
def upsert_user_keyword(db: Session, keyword: str, source: str, active: bool = True) -> UserKeyword:
    normalized = normalize_user_keyword(keyword)
    if not valid_user_keyword(normalized):
        raise HTTPException(status_code=400, detail="Keyword must be 2-40 characters and contain searchable text.")
    entry = db.query(UserKeyword).filter(func.lower(UserKeyword.keyword) == normalized.lower()).first()
    if entry:
        entry.keyword = normalized
        entry.active = active
        entry.source = entry.source or source
        return entry
    entry = UserKeyword(keyword=normalized, active=active, source=source)
    db.add(entry)
    return entry


def article_item(article: Article, similar_articles: list[Article] | None = None, group_info: dict | None = None, related_count_override: int | None = None) -> ArticleListItem:
    similar_articles = similar_articles or []
    group_info = group_info or {}
    group_size = max(1, int(group_info.get("size", 1)))
    related_count = related_count_override if related_count_override is not None else (group_size if group_size > 1 else (len(similar_articles) + 1 if similar_articles else 0))
...
def title_similarity(left: str, right: str) -> float:
    ...
    return title_similarity_from_tokens(left_tokens, right_tokens)


def title_similarity_from_tokens(left_tokens: set[str], right_tokens: set[str]) -> float:
...
def related_articles_for_target(db: Session, target: Article) -> list[Article]:
    ...
    candidates = (
        db.query(Article)
        .filter(Article.region == target.region)
        .order_by(func.coalesce(Article.published_at, Article.fetched_at).desc(), Article.id.desc())
        .limit(1000)
        .all()
    )
    if target.id not in {article.id for article in candidates}:
        candidates.append(target)
    group_map = build_related_groups(candidates)
    target_group_id = group_map.get(target.id, {}).get("id")
    if not target_group_id:
        return [target]
    related = [article for article in candidates if group_map.get(article.id, {}).get("id") == target_group_id]
...
def build_related_groups(articles: list[Article]) -> dict[int, dict]:
    ...
    token_map = {article.id: title_tokens(article.title) for article in articles}
    time_map = {article.id: article_time(article) for article in articles}
    ...
    manual_groups: dict[str, list[int]] = defaultdict(list)
    for article in articles:
        if article.duplicate_group_id:
            manual_groups[article.duplicate_group_id].append(article.id)
    for group_ids in manual_groups.values():
        first_id = group_ids[0]
        for article_id in group_ids[1:]:
            union(first_id, article_id)

    article_by_id = {article.id: article for article in articles}
    token_index: dict[tuple[str, str], list[int]] = defaultdict(list)
    for article in articles:
        candidate_hits: Counter[int] = Counter()
        for token in token_map[article.id]:
            for candidate_id in token_index[(article.region, token)]:
                candidate_hits[candidate_id] += 1
        for candidate_id, overlap in candidate_hits.items():
            if overlap < 3:
                continue
            candidate = article_by_id[candidate_id]
            if should_skip_auto_similarity(article, candidate):
                continue
            if abs((time_map[article.id] - time_map[candidate.id]).days) > 14:
                continue
            if title_similarity_from_tokens(token_map[article.id], token_map[candidate.id]) >= 0.6:
                union(article.id, candidate.id)
        for token in token_map[article.id]:
            token_index[(article.region, token)].append(article.id)
...
def related_view_group_map(target: Article, related_articles: list[Article]) -> dict[int, dict]:
    if not related_articles:
        return {}
    manual_group_id = target.duplicate_group_id or next((article.duplicate_group_id for article in related_articles if article.duplicate_group_id), "")
    representative = next((article for article in related_articles if article.duplicate_group_representative), None) if manual_group_id else None
    if representative is None:
        representative = min(related_articles, key=lambda item: (article_time(item), item.id))
    info = {
        "id": manual_group_id or f"related:{target.id}",
        "label": build_group_label(related_articles),
        "size": len(related_articles),
        "representative_id": representative.id,
        "manual": bool(manual_group_id),
    }
    return {article.id: info for article in related_articles}


def related_count_overrides_for_articles(db: Session, articles: list[Article]) -> dict[int, int]:
    if not articles:
        return {}
    ...
    if manual_group_ids:
        rows = (
            db.query(Article.duplicate_group_id, func.count(Article.id))
            .filter(Article.duplicate_group_id.in_(manual_group_ids))
            .group_by(Article.duplicate_group_id)
            .all()
        )
        manual_counts = {group_id: int(count) for group_id, count in rows}
        for article in articles:
            if article.duplicate_group_id:
                count = manual_counts.get(article.duplicate_group_id, 0)
                overrides[article.id] = count if count > 1 else 0

    regions = {article.region for article in articles if article.region and not article.duplicate_group_id}
    candidates_by_region: dict[str, list[Article]] = {}
    ...
    group_maps_by_region = {region: build_related_groups(candidates) for region, candidates in candidates_by_region.items()}
    for article in articles:
        if article.id in overrides or article.duplicate_group_id:
            continue
        group_map = group_maps_by_region.get(article.region, {})
        group_info = group_map.get(article.id, {})
        group_size = int(group_info.get("size", 1))
        overrides[article.id] = group_size if group_size > 1 else 0
    return overrides
...
def latest_articles_seen_map(db: Session) -> dict[str, int]:
    raw_value = app_setting_value(db, LATEST_ARTICLES_SEEN_SETTING_KEY, "{}")
    try:
    ...
    if not isinstance(value, dict):
        return {}
    seen: dict[str, int] = {}
    for raw_id, raw_seen_at in value.items():
        try:
            article_id = int(raw_id)
            seen_at = int(raw_seen_at)
        except (TypeError, ValueError):
            continue
        if article_id > 0 and seen_at > 0:
            seen[str(article_id)] = seen_at
    return dict(sorted(seen.items(), key=lambda item: item[1])[-LATEST_ARTICLES_SEEN_MAX:])


def save_latest_articles_seen_map(db: Session, seen: dict[str, int]) -> dict[str, int]:
    compacted = dict(sorted(seen.items(), key=lambda item: item[1])[-LATEST_ARTICLES_SEEN_MAX:])
    row = db.get(AppSetting, LATEST_ARTICLES_SEEN_SETTING_KEY) or AppSetting(key=LATEST_ARTICLES_SEEN_SETTING_KEY)
    row.value = json.dumps(compacted, ensure_ascii=False)
    db.add(row)
    db.commit()
    return compacted
...
def list_articles(
    ...
    if related_to:
        ...
        group_map = related_view_group_map(target, related_articles)
    ...
    related_count_map = related_count_overrides_for_articles(db, articles)
    return ArticleListResponse(items=[article_item(article, similar_map.get(article.id, []), group_map.get(article.id), related_count_map.get(article.id)) for article in articles], total=total)
...
@router.get("/keyword-dictionary", response_model=KeywordDictionaryRead)
def keyword_dictionary(db: Session = Depends(get_db)):
    entries = db.query(UserKeyword).order_by(UserKeyword.active.desc(), UserKeyword.keyword.asc()).all()
    counts = active_keyword_counts(db, entries, hours=24)
    return KeywordDictionaryRead(
        items=[user_keyword_read(entry, counts.get(entry.id, 0)) for entry in entries],
        recommendations=keyword_recommendations(db),
    )


@router.post("/keyword-dictionary", response_model=UserKeywordRead)
def add_user_keyword(payload: UserKeywordPayload, db: Session = Depends(get_db)):
    entry = upsert_user_keyword(db, payload.keyword, source="manual", active=payload.active)
    db.commit()
    db.refresh(entry)
    counts = active_keyword_counts(db, [entry], hours=24)
    return user_keyword_read(entry, counts.get(entry.id, 0))


@router.post("/keyword-dictionary/adopt", response_model=UserKeywordRead)
def adopt_user_keyword(payload: UserKeywordPayload, db: Session = Depends(get_db)):
    entry = upsert_user_keyword(db, payload.keyword, source="recommended", active=True)
    db.commit()
    db.refresh(entry)
    counts = active_keyword_counts(db, [entry], hours=24)
    return user_keyword_read(entry, counts.get(entry.id, 0))
...
@router.patch("/keyword-dictionary/{keyword_id}", response_model=UserKeywordRead)
def update_user_keyword(keyword_id: int, payload: UserKeywordUpdatePayload, db: Session = Depends(get_db)):
    entry = db.get(UserKeyword, keyword_id)
    if not entry:
...
@router.delete("/keyword-dictionary/{keyword_id}")
def delete_user_keyword(keyword_id: int, db: Session = Depends(get_db)):
    entry = db.get(UserKeyword, keyword_id)
    if not entry:
        raise HTTPException(status_code=404, detail="Keyword not found")
    db.delete(entry)
...
def dashboard_summary(db: Session = Depends(get_db)):
    ...
    keyword_rows = automatic_recent_keywords(db, hours=24, limit=20)
    chart = [
...
@router.get("/latest-articles/seen", response_model=LatestArticleSeenRead)
def latest_articles_seen(db: Session = Depends(get_db)):
    return LatestArticleSeenRead(seen=latest_articles_seen_map(db))


@router.post("/latest-articles/seen", response_model=LatestArticleSeenRead)
def mark_latest_articles_seen(payload: LatestArticleSeenUpdate, db: Session = Depends(get_db)):
    ids = sorted({int(article_id) for article_id in payload.article_ids if int(article_id) > 0})
    seen = latest_articles_seen_map(db)
    if ids:
        now_ms = int(time() * 1000)
        for article_id in ids:
            seen[str(article_id)] = now_ms
    return LatestArticleSeenRead(seen=save_latest_articles_seen_map(db, seen))
...
def get_app_settings(db: Session = Depends(get_db)):
    ...
    return SettingsRead(
        ...
        enable_alert_domestic_all=values.get("enable_alert_domestic_all", str(settings.enable_alert_domestic_all)).lower() == "true",
        enable_alert_domestic_bok=values.get("enable_alert_domestic_bok", str(settings.enable_alert_domestic_bok)).lower() == "true",
        enable_alert_global=values.get("enable_alert_global", str(settings.enable_alert_global)).lower() == "true",
...
def update_app_settings(payload: SettingsUpdate, db: Session = Depends(get_db)):
    ...
    if {"report_email_enabled", "report_email_recipients"} & set(changes):
        final_email_enabled = bool(changes.get("report_email_enabled", app_setting_bool(db, "report_email_enabled", settings.report_email_enabled)))
        final_recipients = parse_list(changes.get("report_email_recipients", app_setting_value(db, "report_email_recipients", settings.report_email_recipients)))
        if final_email_enabled and not final_recipients:
            raise HTTPException(status_code=400, detail="자동 이메일 발송을 켜려면 받는 사람 이메일을 먼저 추가해야 합니다.")
